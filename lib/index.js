 ...
 * @param {string} [options.postprocessor] - A custom postprocessor like `./postprocessors/po`.
 * @param {object} [options.babylonConfig] - A custom babylon configuration.
 */
export const exportTranslationKeys = async ({rootPath, filePath, filter, logger, progress, preprocessor, postprocessor, babylonConfig}) => {
  const { templates } = await exportTranslationKeysFromFiles({rootPath, filePath, filter, logger, progress, preprocessor, postprocessor, babylonConfig})
  return templates
}
