...
import Ajv from 'ajv'
import {logInfo, logTrace, logSuccess, logWarn, logError} from './logging'
...
function saveSchema({srcPath, templates, schemaPath, callback, logger}) {
  ...
  if (templates && templates.length) {   
    ...
    if (schemaPath) {
      ...
      logSuccess(logger, successMessage)
      callback(0, newSchema)      
    }
  } else {
    ...
    if (schemaPath) {
      ...
      logWarn(logger, message)
      logSuccess(callback, successMessage)
      callback(0, {}) 
    }
  }
}
...
/**
 * validateSchema validates your translations agains a JSON schema.
 * 
 * @export
 * @param {Object} options - The schema generator options.
 * @param {string} options.rootPath - The root directory of your source files.
 ...
 * @param {logger} [options.logger] - A custom logger.
 */
export const validateSchema = ({rootPath, schemaPath, callback, logger}) => {
  try {
 ...
 * @param {logger} [options.logger] - A custom logger.
 */
export const exportTranslationKeys = ({rootPath, filePath = '.', filter = defaultFileFilter, callback, logger}) => {
  if(!rootPath) {
 ...
 * @param {logger} [options.logger] - A custom logger.
 */
export default function ({ srcPath, schemaPath, filter = defaultFileFilter, logger, callback }) {
  if(!srcPath) {
