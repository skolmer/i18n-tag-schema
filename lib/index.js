...
function _vaidateSchema(filePath, schemaPath, callback) {
...
export const vaidateSchema = (rootPath, schemaPath, callback) => {
    try {
        ...
        let tp = 'success'
        if (fileStats.isFile()) {
            _vaidateSchema(rootPath, schemaPath, callback)
            ...
            const readFiles = (dir) => fs.readdir(dir, (err, files) => {
                ...
                files.forEach((file) => {
                    ...
                    if (file.match(/\.json$/)) {
                        totalCount++
                        _vaidateSchema(filePath, schemaPath, (message, type) => {
                            if (callback) {
...
function _exportTranslationKeys(rootPath, filePath, grouped, logger, callback) {
    const templates = []
...
export const exportTranslationKeys = (rootPath, filePath, grouped, logger, callback) => {
    ...
    let fileCount = 0    
    if (fileStats.isFile()) {
        _exportTranslationKeys(rootPath, fullPath, grouped, logger, callback)
    } else if (fileStats.isDirectory()) {        
        const readFiles = (dir) => fs.readdir(dir, (err, files) => {
            if (err) {
            ...
            files.forEach((file) => {                
                ...
                if (file.match(/\.jsx?$/)) {
                    totalCount++
                    _exportTranslationKeys(rootPath, filePath, grouped, logger, (tmpls) => {
                        fileCount++
