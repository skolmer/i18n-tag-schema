...
function jsonDiff(oldJson, newObj, callback) {
    if (oldJson) {
        try {
            let oldJSON = JSON.parse(oldJson)
            let oldKeys = []
            if(oldJSON.definitions && oldJSON.definitions.translations && oldJSON.definitions.translations.properties) {
                oldKeys = Object.keys(oldJSON.definitions.translations.properties)
            } else if(oldJSON.properties) {
                Object.keys(oldJSON.properties).forEach((key) => Object.keys(key.properties).forEach((keyB) => { oldKeys.push(keyB) }))
            }
            let newKeys = []
            if(newObj.definitions && newObj.definitions.translations && newObj.definitions.translations.properties) {
                newKeys = Object.keys(newObj.definitions.translations.properties)
            } else if (newObj.properties) {
                Object.keys(newObj.properties).forEach((key) => Object.keys(key.properties).forEach((keyB) => { newKeys.push(keyB) }))
            }
            oldKeys = oldKeys.filter((name) => name !== '$schema')
            newKeys = newKeys.filter((name) => name !== '$schema')
