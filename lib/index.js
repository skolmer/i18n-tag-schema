...
function saveSchema(srcPath, templates, schema, grouped, callback) {
    if (templates && templates.length) {
        ...
        const jsonString = `{
            "type": "object",
            "properties": {
                ${templates.map((tmpl) => (grouped) ? `${JSON.stringify(tmpl.group)}: { ${tmpl.items.map(t => addTemplateProp(t))} }` : addTemplateProp(tmpl)  )}
            },
        ...
        files.forEach((file) => {
            ...
            if (ext.match(new RegExp(filter))) {
                ...
                fs.readFile(filePath, 'utf-8', (err, contents) => {
                    var newTemplates = inspectFile(contents)
                    const { length } = newTemplates                   
                    
                    if(length) {
                        if(grouped) {
                            templates.push({ group: path.relative(srcPath, filePath), items: [...newTemplates] })
                        } else {
