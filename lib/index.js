                    ...
                    validate.errors.forEach((error) => {
                        ...
                        switch (error.keyword) {
                            case 'required':                            
                                ...
                                message = `${relativePath} is missing translation ${(isGroup) ? 'group' : 'key'} "${error.params.missingProperty}"${(error.dataPath) ? ` in ${error.dataPath}` : ''}`
                                break
                            case 'minLength':
                                missingKeys++
                            ...
                            case 'additionalProperties':
                                unknownKeys++
                                message = `${relativePath} has unknown translation key or group "${error.params.additionalProperty}"${(error.dataPath) ? ` in ${error.dataPath}` : ''}`
                                break
                            default:
                                message = JSON.stringify(`${relativePath} ${error.message}`)
                                break
                        }
                        log(callback, message, 'warning')
                    })
                    const coverage = (totalKeys.count) ? Math.round(((totalKeys.count - missingKeys) / totalKeys.count) * 100) : 0
                    log(callback, `${relativePath} has ${missingKeys} missing translation(s)${(unknownKeys) ? ` and ${unknownKeys} invalid key(s)` : ''}; ${coverage}% translated.`, 'error')
                } else {
