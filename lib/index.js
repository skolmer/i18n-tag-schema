...
function _templatesFromFile(rootPath, filePath, grouped, logger, callback) {
    const templates = []
...
export const templatesFromFile = (rootPath, filePath, grouped, logger, callback) => {
    ...
    let fileCount = 0
    if (fileStats.isFile()) {
        _templatesFromFile(rootPath, fullPath, grouped, logger, callback)
    } else if (fileStats.isDirectory()) {
        const readFiles = (dir) => fs.readdir(dir, (err, files) => {
            if (err) {
            ...
            files.forEach((file) => {
                ...
                if (file.match(/\.jsx?$/)) {
                    totalCount++
                    _templatesFromFile(rootPath, filePath, grouped, logger, (tmpls) => {
                        fileCount++
