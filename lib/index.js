 ...
 */
export const exportTranslationKeys = ({rootPath, filePath = '.', filter = defaultFileFilter, callback, logger, progress, typescript = false}) => {
  if(!rootPath) {
    const error = 'rootPath is not defined.'
    logError(logger, error)
    if(callback) callback(1, error)
 ...
 */
export const generateTranslationSchema = ({ srcPath, schemaPath, filter = defaultFileFilter, logger, callback, progress, typescript = false }) => {
  if(!srcPath) {
      ...
      files.forEach((file) => {
        ...
        if (file.match(filterRegexp)) {
          ...
          fs.readFile(filePath, 'utf-8', (err, contents) => {
            ...
            const tmpl = readFileTemplates(srcPath, filePath, templates, templatePatterns, contents, logger, typescript)
            templatePatterns = tmpl.templatePatterns
            templates = tmpl.templates
            fileCountRead++
            progressCallback(fileCountRead, fileCount, file)
