...
function jsonDiff(oldJson, newObj, callback) {
    if (oldJson) {
        try {
            ...
            let oldKeys
            if(oldJSON.definitions && oldJSON.definitions.translations) {
                oldKeys = Object.keys(oldJSON.definitions.translations.properties)
            } else {
                let props = []
                oldJSON.properties.forEach(({properties}) => props.push(properties))
                oldKeys = Object.keys(props)
            }
            let newKeys
            if(newObj.definitions && newObj.definitions.translations) {
                newKeys = Object.keys(newObj.definitions.translations.properties)
            } else {
                let props = []
                newObj.properties.forEach(({properties}) => props.push(properties))
                newKeys = Object.keys(props)
            }
    ...
    return { removed: [], added: Object.keys(newObj.properties || newObj.definitions.translations.properties) }
}
