    ...
    if (templates && templates.length) {
        ...
        if(schema) {              
            ...
            fs.writeFile(schema, JSON.stringify(json, null, '\t'), 'utf-8', function (err) {
                ...
                log(callback, `i18n json schema has been updated; contains ${Object.keys(json.properties).length} keys  ( ${diff.added.length} added / ${diff.removed.length} removed ): ${schema}`);
            })
        } else {
            log(callback, json)
        }
