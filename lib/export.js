...
export const exportTranslationKeysFromFiles = async ({rootPath, filePath = '.', filter = /\.jsx?$/, logger, progress, preprocessor, postprocessor, babylonConfig}) => {
  ...
  const progressCallback = (progress) ? throttle(progress, 300, { 'leading': true }) : () => {}
  ...
  if (fileStats.isFile()) {
    ...
    if(fileNames.length) progressCallback(totalCount, totalCount, fileNames[fileNames.length-1].file)
