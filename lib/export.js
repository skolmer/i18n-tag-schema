 ...
 */
const readFileTemplates = ({rootPath, filePath, templates = [], templatePatterns = {}, contents, logger, preprocessor}) => {
  const parsed = parseFile(contents, templatePatterns, preprocessor)
  const {
    templates: {
      length
    },
 ...
 * @param {string} [options.preprocessor] - A custom preprocessor like `preprocessors/typescript`.
 */
export const exportTranslationKeysFromFiles = async ({rootPath, filePath = '.', filter = /\.jsx?$/, logger, progress, preprocessor}) => {
  if(!rootPath) {
  ...
  if (fileStats.isFile()) {
    ...
    const { templates, templatePatterns } = readFileTemplates({rootPath, filePath, templates, contents, logger, preprocessor})
    ...
    for(const fileName of fileNames) {
      ...
      try {
        ...
        const result = readFileTemplates({rootPath, filePath, templates, templatePatterns, contents, logger, preprocessor})
