 ...
 * @param {boolean} [options.typescript] - Set to true if the content is typescript code.
 */
const readFileTemplates = ({rootPath, filePath, templates = [], templatePatterns = {}, contents, logger, typescript}) => {
  const parsed = parseFile(contents, templatePatterns, typescript)
  const {
    templates: {
      length
    },
 ...
 */
export const exportTranslationKeysFromFiles = async ({rootPath, filePath = '.', filter = /\.jsx?$/, logger, progress, typescript}) => {
  if(!rootPath) {
  ...
  if (fileStats.isFile()) {
    ...
    const { templates, templatePatterns } = readFileTemplates({rootPath, filePath, templates, contents, logger, typescript})
    ...
    for(const fileName of fileNames) {
      ...
      try {
        const contents = await fs.readFile(filePath, 'utf-8')
        const result = readFileTemplates({rootPath, filePath, templates, templatePatterns, contents, logger, typescript})
