...
export const readFileTemplates = (rootPath, filePath, templates, templatePatterns, contents, logger, typescript) => {
  try {
    const parsedTemplates = parseFile(contents, typescript)
 ...
 */
export const exportTranslationKeysFromFile = ({rootPath, filePath, callback, logger, typescript = false}) => {
  console.log(typescript || 'ahhhhh')
  const templates = []
  ...
  fs.readFile(filePath, 'utf-8', (err, contents) => {
    ...
    const tmpls = readFileTemplates(rootPath, filePath, templates, templatePatterns, contents, logger, typescript)
